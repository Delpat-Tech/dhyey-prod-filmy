...
const defaultData = {
  overview: {
    ...
    totalStories: { value: 0, change: 0, trend: 'up' },
    totalViews: { value: 0, change: 0, trend: 'up' },
    engagementRate: { value: 0, change: 0, trend: 'up' }
  }
}
...
export default function AnalyticsDashboard() {
  ...
  const [analytics, setAnalytics] = useState<any>(defaultData)
    ...
    <div className="space-y-6">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-3xl font-bold tracking-tight">{formatNumber(analytics.overview?.totalUsers?.value || 0)}</p>
              </div>
            ...
            <div className="flex items-center justify-between text-sm">
              <div className={`flex items-center px-2 py-1 rounded-full ${
                analytics.overview?.totalUsers?.trend === 'up' ? 'bg-green-500/20 text-green-200' : 'bg-red-500/20 text-red-200'
              }`}>
                {analytics.overview?.totalUsers?.trend === 'up' ? (
                  <TrendingUp className="h-3 w-3 mr-1" />
                ) : (
                  <TrendingDown className="h-3 w-3 mr-1" />
                )}
                <span className="font-medium">{analytics.overview?.totalUsers?.change || 0}%</span>
              </div>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-3xl font-bold tracking-tight">{formatNumber(analytics.overview?.totalStories?.value || 0)}</p>
              </div>
              <div className="p-3 bg-white/20 rounded-xl backdrop-blur-sm group-hover:bg-white/30 transition-colors">
            ...
            <div className="flex items-center justify-between text-sm">
              <div className={`flex items-center px-2 py-1 rounded-full ${
                analytics.overview?.totalStories?.trend === 'up' ? 'bg-green-500/20 text-green-200' : 'bg-red-500/20 text-red-200'
              }`}>
                {analytics.overview?.totalStories?.trend === 'up' ? (
                  <TrendingUp className="h-3 w-3 mr-1" />
                ) : (
                  <TrendingDown className="h-3 w-3 mr-1" />
                )}
                <span className="font-medium">{analytics.overview?.totalStories?.change || 0}%</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-300">
          ...
          <div className="space-y-4">
            {analytics.userGrowth && analytics.userGrowth.length > 0 ? analytics.userGrowth.map((data: any, index: number) => (
              <div key={data.month} className="group flex items-center space-x-4 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                    <span>Growth: +{index > 0 ? ((data.users / (analytics.userGrowth[index-1]?.users || data.users)) * 100 - 100).toFixed(1) : 0}%</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-300">
          ...
          <div className="space-y-4">
            {analytics.topGenres && analytics.topGenres.length > 0 ? analytics.topGenres.map((genre: any, index: number) => {
              const colors = [
      ...
      <div className="bg-gradient-to-br from-green-50 to-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-300">
        ...
        <div className="space-y-3">
          {analytics.recentActivity && analytics.recentActivity.length > 0 ? analytics.recentActivity.map((activity: any, index: number) => {
            const typeConfig = {
              user: { 
                bg: 'bg-blue-500', 
                ring: 'ring-blue-200', 
